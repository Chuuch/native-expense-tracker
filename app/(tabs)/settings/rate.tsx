    ...
    <View className={`flex-1 ${colors.background} pt-20`}>
      ...
      <View className='flex-row items-center justify-between w-full p-4'>
        ...
        <View className='flex-row items-center gap-2'>
          <Text className={`${colors.text} text-4xl font-bold`}>Rate App</Text>
        </View>
      ...
      >
        ...
        <View className={`w-full ${colors.card} rounded-xl p-6`}>
          <Text className={`${colors.text} text-xl font-bold mb-4`}>How are we doing?</Text>
        ...
        <View className='w-full'>
          <Text className={`${colors.text} text-xl font-bold mb-4`}>Rate your experience</Text>
              ...
              {ratingOptions.map((option) => (
                ...
                >
                  ...
                  <Text className={`text-sm font-semibold ${
                    rating === option.value ? `text-[${colors.accent}]` : `text-[${colors.text}]`
                  }`}>
                    {option.label}
                  </Text>
                </TouchableOpacity>
        ...
        {rating >= 4 && (
          <View className='w-full'>
            <Text className={`${colors.text} text-xl font-bold mb-4`}>Rate us on the App Store</Text>
            <View className={`${colors.card} rounded-xl p-6`}>
              <Text className={`${colors.textSecondary} text-base mb-6 text-center`}>
        ...
        {rating >= 4 && (
          <View className='w-full'>
            <Text className={`${colors.text} text-xl font-bold mb-4`}>What do you like most?</Text>
                  ...
                  <TouchableOpacity
                    ...
                    className={`px-3 py-2 rounded-full border ${
                      selectedReasons.includes(reason)
                        ? `bg-[${colors.accent}] border-[${colors.accent}]`
                        : 'bg-gray-300 border-gray-300'
                    }`}
                    onPress={() => handleReasonToggle(reason)}
                  >
                    <Text className={`text-sm font-semibold ${
                      selectedReasons.includes(reason) ? `text-[${colors.text}]` : `text-[${colors.textSecondary}]`
                    }`}>
                      {reason}
                    </Text>
                  </TouchableOpacity>
        ...
        {rating >= 4 && (
          <View className='w-full'>
              <Text className={`${colors.text} text-xl font-bold mb-4`}>Tell us more (Optional)</Text>
            <View className={`${colors.card} rounded-xl p-4`}>
              <TextInput
                className={`${colors.text} text-base min-h-[100px]`}
        ...
        {rating > 0 && rating <= 3 && (
          <View className='w-full'>
            <Text className={`${colors.text} text-xl font-bold mb-4`}>How can we improve?</Text>
            <View className={`${colors.card} rounded-xl p-4`}>
              <TextInput
                className={`${colors.text} text-base min-h-[100px]`}
        ...
        <View className='w-full'>
          <TouchableOpacity 
            className={`rounded-xl p-4 items-center ${
              rating > 0 ? `bg-[${colors.accent}]` : `${colors.cardSecondary}`
            }`}
            onPress={handleSubmitFeedback}
            disabled={rating === 0}
          >
            <Text className={`text-lg font-semibold ${
              rating > 0 ? `text-[${colors.text}]` : `text-white`
            }`}>
        ...
        {/* Share App */}
        <View className='w-full'>
          <Text className={`${colors.text} text-xl font-bold mb-4`}>Share with Friends</Text>
          <View className={`${colors.card} rounded-xl p-6`}>
            <Text className={`${colors.textSecondary} text-base mb-4 text-center`}>
              Love the app? Share it with friends and family who might find it useful!
            </Text>
            <TouchableOpacity className={`${colors.accent} rounded-xl p-4 items-center`}>
              <Text className='text-white text-lg font-semibold'>Share App</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
