  ...
  return (
    <View className={`flex-1 ${colors.background} pt-20`}>
      ...
      <View className="flex-row items-center justify-between w-full p-4">
        ...
        <Text className={`${colors.text} text-4xl font-bold`}>Appearance</Text>
      ...
      >
            ...
            {themes.map((theme) => (
              <TouchableOpacity
                ...
                className={`w-full ${colors.card} rounded-xl p-4 border-2 ${
                  selectedTheme === theme.id ? 'border-[#CBFD03]' : colors.border
                }`}
                ...
                <View className="flex-row items-center justify-between">
                  <View className="flex-row items-center gap-4 flex-1">
                    <View className={`${theme.colors.accent} rounded-full p-3`}>
                      <Feather name={theme.id === 'auto' ? 'smartphone' : theme.id === 'oled' ? 'monitor' : theme.id === 'light' ? 'sun' : 'moon'} size={24} color="black" />
                    </View>
                    <View className="flex-1">
          ...
          <View className={`${colors.card} rounded-xl p-6`}>
            ...
            <View className="items-center">
              {(() => {
                ...
                return (
                  <View className={`${selectedThemeOption.colors.background} rounded-xl p-6 border-2 border-[#CBFD03] w-full max-w-sm`}>
                    {/* Header */}
                    ...
                    <View className={`${selectedThemeOption.colors.card} rounded-xl p-4 mb-4`}>
                      ...
                      <Text className={`${selectedThemeOption.colors.accent} text-2xl font-bold mb-2`}>$2,450.75</Text>
                      <View className="flex-row items-center gap-2">
                        <Feather name="trending-up" size={16} color={selectedThemeOption.colors.accent.includes('CBFD03') ? '#CBFD03' : '#22c55e'} />
                        <Text className={`${selectedThemeOption.colors.accent} text-sm font-semibold`}>+$125.50</Text>
                      </View>
                    </View>
        ...
        {/* Save Button */}
        <TouchableOpacity 
          className="w-full bg-[#CBFD03] rounded-xl p-4 items-center mb-20"
